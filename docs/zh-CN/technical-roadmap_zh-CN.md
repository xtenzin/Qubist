...
- 搜索结果展示：显示匹配的点、相似度分数和 payload 信息

#### 1.2 Ollama 集成
- 页面集成 Ollama 服务
- 支持用户输入文本，调用嵌入模型生成向量
- 在添加、编辑、搜索功能中都可以使用 Ollama：
  - **添加点**：用户输入文本，通过 Ollama 生成向量
  - **编辑点**：用户输入文本，通过 Ollama 更新向量
  - **搜索**：用户输入文本，通过 Ollama 生成查询向量进行搜索

### 2. 集合字段分析功能

- 分析集合中 payload 的字段构成
- 显示字段的数据类型
- 分析范围：仅分析当前页数据，避免大数据量性能问题
- 提供字段统计信息展示界面
